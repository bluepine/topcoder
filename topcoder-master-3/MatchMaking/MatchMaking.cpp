...
public:
    string makeMatch(vector <string> namesWomen, vector <string> answersWomen, vector <string> namesMen, vector <string> answersMen, string queryWoman) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            for (int i = 0; i < N; ++i) {
                if (rw[i]) continue;
                if (i_min == -1 || namesWomen[i] < namesWomen[i_min]) {
                    i_min = i;
                }
            }
            ...
            for (int j = 0; j < N; ++j) {
                if (rm[j]) continue;
                int c = 0;
                for (int k = 0; k < (int)answersWomen[i_min].size(); ++k) {
                    if (answersWomen[i_min][k] == answersMen[j][k])
                        c++;
                }
                if (c > c_max || (c == c_max && namesMen[j] < namesMen[j_max])) {
                    c_max = c;
                    j_max = j;
                }
            }
            ...
            if (namesWomen[i_min] == queryWoman) {
                return namesMen[j_max];
            }
        }
        return "";
    }
