...
//
// END CUT HERE
#line 174 "SkewedPerspectives.cpp"
#include <string>
#include <vector>
#include <iostream>
#include <sstream>
...
bool solve1(vector<int> cubes, int B, int w, string view)
{
    int n = view.size();
    bool valid = true;
    int i;
    int wi = w - 1;
    vector<int> tofill(w, 0);

    for (i = n - 1; i >= 0; i--) {
        int increase = 0;
        int changetower = false;
        int block = view[i] - '0';
        ...
        if (block >= 0 && block <= 2) {
            ...
            } else if (i < n - 1 && view[i + 1] == 'b') { // last element
                tofill[wi] = 1;
                increase = 1;
                changetower = true;
            } else {
...
class SkewedPerspectives
    {
    public:
    vector <string> areTheyPossible(const vector <int> cubes, const int B, const int w, const vector <string> views)
        {
            int vi;
            int vn = views.size();
            vector<string> ret(vn);

            for (vi = 0; vi < vn; vi++) {
                ret[vi] = solve1(cubes, B, w, views[vi])? "valid": "invalid";
            }
            return ret;
        }
...
// BEGIN CUT HERE
	...
	void test_case_0() { int Arr0[] = {100, 99, 85}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arg1 = 91; int Arg2 = 11; string Arr3[] = { "bbb10112000b02bbb0bbb1bb200b1b020b02b00b0b000021b0", }; vector <string> Arg3(Arr3, Arr3 + (sizeof(Arr3) / sizeof(Arr3[0]))); string Arr4[] = { "valid",  }; vector <string> Arg4(Arr4, Arr4 + (sizeof(Arr4) / sizeof(Arr4[0]))); verify_case(0, Arg4, areTheyPossible(Arg0, Arg1, Arg2, Arg3)); }
